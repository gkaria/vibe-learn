...
1. Ledger: `touch` each concept the tour covered, one call per concept, using a stable kebab-case name and a short human label — `vibe-learn-knowledge touch <name> --label="<Human label>"`, or `bash <path>/knowledge.sh touch ...` via the lookup order above. Never hand-edit the JSON.
