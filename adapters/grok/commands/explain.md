...
**Knowledge helper:** to mark concepts as seen, use the `knowledge.sh` helper via `run_terminal_command` — never hand-edit the JSON. Locate it in this order:

1. `~/.vibe-learn/scripts/knowledge.sh` (global install)
2. The install directory this project's hooks point at: read the vibe-learn hook command from `.grok/hooks/vibe-learn.json` (or `~/.grok/hooks/vibe-learn.json`) — it ends in `<install-dir>/scripts/bootstrap.sh`, and `knowledge.sh` sits in the same directory
...
3. Offer the next step: "Want me to quiz you on this (`/quiz <topic>`), or save it to Obsidian (`/learn obsidian <topic>`)?"
